extends Node2D  # Script for PillarA2

@export var color_name: String = "Red"                  # Pillar color
@export var matching_pillars: Array[NodePath] = []      # Pillars it corresponds to (e.g., PillarA1)
...
func _on_body_entered(body):
	if body.name == "Player":
		player_in_area = true
		# Slightly light up linked pillars
		for path in matching_pillars:
			var pillar = get_node_or_null(path)
			if pillar and pillar.has_node("BaseSprite2"):
				var sprite = pillar.get_node("BaseSprite2")
				if not pillar.activated:
					sprite.modulate = get_color_preview(sprite.modulate)

func _on_body_exited(body):
	if body.name == "Player":
		player_in_area = false
		# Revert linked pillars to dark if not activated
		for path in matching_pillars:
			var pillar = get_node_or_null(path)
			if pillar and pillar.has_node("BaseSprite2"):
				var sprite = pillar.get_node("BaseSprite2")
				if not pillar.activated:
					sprite.modulate = get_color_dark(pillar.color_name)

func activate():
	activated = true
	base_sprite.modulate = get_color_bright(color_name)

	# Fully activate linked pillars
	for path in matching_pillars:
		var pillar = get_node_or_null(path)
		if pillar and pillar.has_node("BaseSprite2"):
			pillar.activated = true
			pillar.get_node("BaseSprite2").modulate = get_color_bright(pillar.color_name)
...
# Slightly brightened preview
func get_color_preview(original: Color) -> Color:
	return original.lightened(0.3)
